...
class AudioManager:
    ...
    def _load(self):
        self.s = {}
        files = {
            "ambient_loop":           "sounds/ambient_loop.wav",
            "calibration_music":      "sounds/calibration_ready.wav",
            "calibration_voice":      "sounds/cabiration_ready_word_audio.ogg",
            "hold_start":             "sounds/hold_start.ogg",
            "charging_loop":          "sounds/charging_loop.ogg",
            "merge_event":            "sounds/merge_event.ogg",
            "warning":                "sounds/warning.ogg",
            "session_complete":       "sounds/session_complete.ogg",
            "proximity_close":        "sounds/proximity_close.ogg",
            "singularity_merged":     "sounds/singularity_merged.ogg",
        }
...
def draw_text_orbitron(frame, text, pos, font_size, color_bgr, weight="Bold", align="center",
    ...
    if font_size not in _orbitron_cache:
        font = ImageFont.truetype("fonts/Orbitron.ttf", font_size)
        try:
            font.set_variation_by_name(weight)
        except Exception:
            pass
        _orbitron_cache[font_size] = font
    font = _orbitron_cache[font_size]
...
def draw_metallic_title(frame, text, center_xy, font_size, weight="Bold",
                         box_color_bgr=(190,150,30), box_padding=(28,14)):
    if font_size not in _orbitron_cache:
        font = ImageFont.truetype("fonts/Orbitron.ttf", font_size)
        try:
            font.set_variation_by_name(weight)
...
def draw_intro_screen(frame, blink_on=True, intro_image_right=None, intro_image_left=None):
    ...
    def centered_at(text, cx, y, font, scale, color, thick=1):
        (tw, th), _ = cv2.getTextSize(text, font, scale, thick)
        cv2.putText(frame, text, (cx - tw//2, y), font, scale, color, thick, cv2.LINE_AA)

    # Title
    centered("BLACK HOLE SURGEON", H//2-260, cv2.FONT_HERSHEY_DUPLEX, 1.3, C_PURPLE, 2)

    # Subtitle
    centered("Exercise: Gomukhasana (Cow Face Pose)", H//2-210,
              cv2.FONT_HERSHEY_SIMPLEX, 0.65, C_CYAN, 1)

    # Narrative
    lines = [
        "The stars and galaxies of the universe are in danger.",
        "Two singularities are drifting toward collision,",
        "threatening to tear the fabric of spacetime apart.",
        "",
        "Only you, the Surgeon, can guide them together",
        "and seal the rift -- safely, with steady hands.",
    ]
    y = H//2 - 150
    for line in lines:
        if line:
            centered(line, y, cv2.FONT_HERSHEY_SIMPLEX, 0.58, C_WHITE, 1)
        y += 34

    # Fact box
    box_w, box_h = 640, 90
    bx, by = W//2 - box_w//2, y + 18
    ov2 = frame.copy()
    cv2.rectangle(ov2,(bx,by),(bx+box_w,by+box_h),(10,4,25),-1)
    cv2.addWeighted(ov2,0.7,frame,0.3,0,frame)
    cv2.rectangle(frame,(bx,by),(bx+box_w,by+box_h),C_GRID_HOT,1)

    fy = by + 26
    centered("DID YOU KNOW?", fy, cv2.FONT_HERSHEY_DUPLEX, 0.5, C_GOLD, 1)
    fy += 26
    centered("This is the same shoulder reach you use to scratch your back", fy,
              cv2.FONT_HERSHEY_SIMPLEX, 0.42, (200,200,200), 1)
    fy += 22
    centered("or grab something off a high shelf -- now you're training it directly.", fy,
              cv2.FONT_HERSHEY_SIMPLEX, 0.42, (200,200,200), 1)

    box_bottom = by + box_h
    dia_cy = by + box_h // 2

    def blend_image(img, cx):
        img_h, img_w = img.shape[:2]
        x0, y0 = cx - img_w // 2, dia_cy - img_h // 2
    ...
    if intro_image_right is not None:
        blend_image(intro_image_right, ((bx + box_w) + W) // 2)
    if intro_image_left is not None:
        blend_image(intro_image_left, (0 + bx) // 2)
    # Blinking prompt — anchored to fact box, not the diagram
    if blink_on:
        centered("Press SPACE to begin", box_bottom + 70,
                  cv2.FONT_HERSHEY_DUPLEX, 0.75, C_GOLD, 2)                
        
def draw_session_summary(frame, m: Metrics, streak=0, intro_image_right=None, intro_image_left=None):
    ...
    draw_text_orbitron(frame, "SESSION SUMMARY", (W//2, 172), 26, C_PURPLE, weight="Medium")
    ...
    def section(title, rows, y_start):
        ...
        for i,(lbl,val,col,note,pct) in enumerate(rows):
            ...
            cv2.putText(frame,lbl,(bx+24,ry),
                        cv2.FONT_HERSHEY_SIMPLEX,0.48,(170,150,200),1,cv2.LINE_AA)
            (vw,_),_ = cv2.getTextSize(val, cv2.FONT_HERSHEY_DUPLEX, 0.62, 1)
            cv2.putText(frame,val,(bx+box_w-24-vw,ry),
                        cv2.FONT_HERSHEY_DUPLEX,0.62,col,1,cv2.LINE_AA)
            if note:
                cv2.putText(frame,note,(bx+24,ry+18),
                            cv2.FONT_HERSHEY_SIMPLEX,0.34,(130,120,150),1,cv2.LINE_AA)
            if pct is not None:
    ...
    perf_rows = [
        ...
        ("TOTAL REPS",       str(m.reps),    C_WHITE, None, None),
        ("BEST REACH GAP",   f"{m.best_gap:.2f}", gap_color, "lower = better", None),
        ("BEST HOLD",        f"{m.best_hold_s:.1f}s", hold_color, None, None),
    ]
    ...
    mm,ss = divmod(int(m.session_time()),60)
    centered(f"Session time {mm:02d}:{ss:02d}   |   {streak} day streak",
             next_y + 36, cv2.FONT_HERSHEY_SIMPLEX, 0.46, (170,170,170), 1)
    ...
    centered("Press R to play again   |   Q to quit", H-38,
...
class BlackHoleSurgeon:
    ...
    def __init__(self):
        ...
        self.intro_image_right = cv2.imread("images/black_hole_icon.png")
        self.intro_image_left  = cv2.imread("images/black_hole_icon_left.png")   # ← new
    ...
    def run(self):
        while True:
            ...
            data = None
            if self.phase not in (Phase.PAUSED, Phase.SESSION_END, Phase.INTRO):
            ...
            if self.phase == Phase.INTRO:
                ...
                draw_intro_screen(frame, blink_on=blink_on, intro_image_right=self.intro_image_right, intro_image_left=self.intro_image_left)
            ...
            if self.phase == Phase.SESSION_END:
                ...
                draw_session_summary(frame, self.metrics, self.current_streak,
                                      self.intro_image_right, self.intro_image_left)
                cv2.imshow("Black Hole Surgeon", frame)
                continue
