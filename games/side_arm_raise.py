"""
RehabVerse v4 — Rotator Cuff Recovery Tracker (Single-Arm / 8-Week AM-PM Program)
══════════════════════════════════════════════════════════════════════════════
WHAT CHANGED FROM v3
  1. SINGLE-ARM MODE
     Your frontend collects the "surgery hand". That value is persisted in
     rehabverse_data.json under "surgery_arm" ("L" or "R"). Only that arm's
     panel is shown — no more split L/R panels. Set it once via:
         python rehabverse_v4.py L      (or R)
     ...or just answer the one-time prompt the first time you run it.

  2. 8-WEEK PROGRAM, TWICE-DAILY (AM / PM) SESSIONS
     Each week has ONE fixed ROM target for all 7 days.
     Hold-time and Rep targets ramp up smoothly across the week:
        • Hold-time increases once per DAY (applied to both the AM and PM
          session of that day) — "morning session increases hold".
        • Rep count increases once per EVENING — the PM target reps is
          higher than that day's AM target reps — "evening increases reps".
     Day 1 starts near last week's finishing numbers (week 1 starts at 50%
     of its own target) and ramps linearly to the week's final numbers by
     Day 7 PM.
     You only advance to the next week once the Day-7-PM session actually
     MEETS the week's full target (rom + hold + reps). If you don't hit it,
     you simply repeat the Day-7-PM target next session — the program will
     not silently move you forward.

  3. MISSED-SESSION ADAPTATION
     If you skip one or more full calendar days between sessions, the very
     next session's hold & rep targets are reduced (2s per missed slot for
     hold, 1 rep per missed slot for reps, floored so it never asks for
     something silly). The HUD flags this as a "SETBACK" so you know why
     the bar moved.

  4. ACCURATE PER-WEEK STICK FIGURE
     The bottom-bar illustration now raises the stick figure's arm to the
     EXACT angle math (0°=arm straight down, 90°=forward horizontal,
     180°=straight overhead) that the week is targeting — using the same
     angle convention as the real ROM measurement — with a small prop
     (pendulum / table / wall marks / shelf / overhead bar) that matches
     real rotator-cuff PT stages for that ROM range.

  5. CLEANER LAYOUT
     Bigger single side panel (320px), a dedicated "SESSION TARGET" box so
     rom/hold/reps targets are always visible, generous vertical spacing
     everywhere the old build overlapped text, and a redesigned bottom bar
     (roadmap grid + illustration) that no longer collides with the log.

Tune anything under "TUNABLE CONSTANTS" below.
"""
